...
var deleteInputMapping = map[string]string{
	"AlertSourceServiceDeleteInput": "ID",
	"CategoryDeleteInput":           "ID",
	"CheckDeleteInput":              "ID",
	"DeleteInput":                   "ID",
	"LevelDeleteInput":              "ID",
	"TagDeleteInput":                "ID",
	"ToolDeleteInput":               "ID",
	"TeamDeleteInput":               "string",
	"IdentifierInput":               "string",
}

func deleteInputRename(inputValue *types.InputValueDefinition) string {
	inputType := strings.TrimSuffix(inputValue.Type.String(), "!")
	if convertedValue, ok := deleteInputMapping[inputType]; ok {
		return convertedValue
	}
	return inputType
}

func doDebug(dbg any) string {
	return ""
}

func genMutations(objects map[string]*types.ObjectTypeDefinition) {
	var buf bytes.Buffer

	buf.WriteString(header)

	tmpl := template.New("mutations")
	tmpl.Funcs(sprig.TxtFuncMap())
	tmpl.Funcs(templFuncMap)
	template.Must(tmpl.ParseFiles("./templates/mutations.tpl"))

	if err := tmpl.ExecuteTemplate(&buf, "create_operation", objects["Mutation"]); err != nil {
		panic(err)
	}
	if err := tmpl.ExecuteTemplate(&buf, "update_operation", objects["Mutation"]); err != nil {
		panic(err)
	}
	if err := tmpl.ExecuteTemplate(&buf, "delete_operation", objects["Mutation"]); err != nil {
		panic(err)
	}

	fmt.Println("writing mutation.go")
	err := os.WriteFile("mutation.go", buf.Bytes(), 0o644)
	if err != nil {
		panic(err)
	}
}
...
var templFuncMap = template.FuncMap{
	...
	"quote":                               strconv.Quote,
	"doDebug":                             doDebug,
	"deleteInputRename":                   deleteInputRename,
