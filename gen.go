...
type {{.Name}} struct {
    {{ range .Fields }}
  {{.Name | title}} {{ if hasSuffix "s" .Name }}[]{{- end -}}
    {{ if eq (.Name | title) "Errors" }}Error
    {{- else if eq (.Name | title) "Warnings" }}Warning
    {{- else }}{{ .Name | title | convertPayloadType }}
    {{- end }} // {{.Description | clean | fullSentence}}
    {{- end }}
}
{{- end }}{{ end -}}
`),
...
func (client *Client) {{ .Name | title }}(input any) (*AlertSource, error) {
    var q struct {
      Account struct {
        {{ .Name | title }} {{ .Name | title }} ` + "`" + `graphql:"{{.Name}}(input: $input)"` + "`" + `
      }
    }
    ...
    err := client.Query(&q, v, WithName("{{.Name}}Get"))
...
type {{.Name}} struct { {{range .Fields }}
  {{.Name | title}} *{{ .Name | title | convertPayloadType }}  // {{.Description | clean | fullSentence}}
 {{- end }}
}
{{- end }}
...
var templFuncMap = template.FuncMap{
	...
	"quote":    strconv.Quote,
	"join":     strings.Join,
	"convertPayloadType": func(s string) string {
		value := strings.ToLower(s)
		if strings.HasSuffix(value, "alias") || strings.HasSuffix(value, "email") {
			return "string"
		}
