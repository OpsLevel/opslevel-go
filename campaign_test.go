...
func TestListCampaigns(t *testing.T) {
	...
	testRequestOne := autopilot.NewTestRequest(
		`query CampaignsList($after:String!$first:Int!$sortBy:CampaignSortEnum!){account{campaigns(first: $first, after: $after, sortBy: $sortBy){nodes{checkStats{total,totalSuccessful},endedDate,filter{id,name},htmlUrl,id,name,owner{alias,id},projectBrief,rawProjectBrief,reminder{channels,daysOfWeek,defaultSlackChannel,frequency,frequencyUnit,message,nextOccurrence,timeOfDay,timezone},serviceStats{total,totalSuccessful},startDate,status,targetDate},pageInfo{hasNextPage,hasPreviousPage,startCursor,endCursor}}}}`,
		`{ "after": "", "first": 100, "sortBy": "start_date_DESC" }`,
	...
	testRequestTwo := autopilot.NewTestRequest(
		`query CampaignsList($after:String!$first:Int!$sortBy:CampaignSortEnum!){account{campaigns(first: $first, after: $after, sortBy: $sortBy){nodes{checkStats{total,totalSuccessful},endedDate,filter{id,name},htmlUrl,id,name,owner{alias,id},projectBrief,rawProjectBrief,reminder{channels,daysOfWeek,defaultSlackChannel,frequency,frequencyUnit,message,nextOccurrence,timeOfDay,timezone},serviceStats{total,totalSuccessful},startDate,status,targetDate},pageInfo{hasNextPage,hasPreviousPage,startCursor,endCursor}}}}`,
		`{ "after": "OA", "first": 100, "sortBy": "start_date_DESC" }`,
	...
	// Act
	response, err := client.ListCampaigns(nil, nil)
	...
	autopilot.Equals(t, ol.CampaignStatusEnumEnded, result[1].Status)
	autopilot.Equals(t, "Campaign 3", result[2].Name)
	autopilot.Equals(t, ol.CampaignStatusEnumDraft, result[2].Status)
