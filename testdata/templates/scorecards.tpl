...
mutation ScorecardCreate($input:ScorecardInput!){scorecardCreate(input: $input){scorecard{id,aliases,affectsOverallServiceLevels,description,filter{id,name,connective,htmlUrl,predicates{caseSensitive,key,keyData,type,value}},href,name,owner{... on Team{teamAlias:alias,id}},passingChecks,passingChecksPercentage,serviceCount,servicesReport{levelCounts{level{alias,description,id,index,name},serviceCount}},slug,totalChecks},errors{message,path}}}
{{ end }}
...
query ScorecardGet($input:IdentifierInput!){account{scorecard(input: $input){id,aliases,affectsOverallServiceLevels,description,filter{id,name,connective,htmlUrl,predicates{caseSensitive,key,keyData,type,value}},href,name,owner{... on Team{teamAlias:alias,id}},passingChecks,passingChecksPercentage,serviceCount,servicesReport{levelCounts{level{alias,description,id,index,name},serviceCount}},slug,totalChecks}}}
{{ end }}
